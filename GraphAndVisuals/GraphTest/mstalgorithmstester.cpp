...
namespace graph
{
	...
	void MSTAlgorithmsTester::assertEqual(EdgeMap &edgeMapA, EdgeMap &edgeMapB)
	{
		...
		for(EdgeMap::const_iterator itA = edgeMapA.begin(), endA = edgeMapA.end(); itA != endA; ++itA)
		{
			...
			{
				if(std::get<0>(*itA)->getDistance() != std::get<0>(*itB)->getDistance())
					continue;

				if((void *) std::get<1>(*itA) != (void *) std::get<1>(*itB) && (void *) std::get<1>(*itA) != (void *) std::get<2>(*itB))
					continue;

				if((void *) std::get<2>(*itA) != (void *) std::get<1>(*itB) && (void *) std::get<2>(*itA) != (void *) std::get<2>(*itB))
					continue;
